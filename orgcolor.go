...
// dimTint is how much of an org's colour survives when it has been filtered
// out. Mixed into the background rather than made transparent, so it stays flat
// against whatever it is drawn over.
const dimTint = 0.25

// dimmed is the washed-out form of a colour, used everywhere a filtered-out org
// still holds its place: the dot in the key, and the dot and bar on the
// report's split. One function so "switched off" looks the same on every tab.
func dimmed(c color.Color) color.NRGBA {
	return blendColor(theme.Color(theme.ColorNameBackground), c, dimTint)
}

// orgDot is the colour key that goes beside a name or a number.
func orgDot(cfg Config, org string) fyne.CanvasObject {
	return orgDotIn(orgColor(cfg, org))
}

// orgDotIn is orgDot in a colour the caller chose, so a filtered-out org can
// show the same washed-out dot the key uses.
func orgDotIn(fill color.Color) fyne.CanvasObject {
	d := canvas.NewRectangle(fill)
	d.CornerRadius = 5
	d.SetMinSize(fyne.NewSize(10, 10))
	return container.NewCenter(d)
}

// shareBar is one org's share of a total as a bar in a chosen colour. The
// report's split is a row of these — a meter each, rather than one stacked bar
// — and the colour is passed in so a hidden row can be drawn greyed instead of
// vanishing, which would take the row's size away with it.
func shareBar(fill color.Color, mins, total int, height float32) fyne.CanvasObject {
	track := canvas.NewRectangle(theme.Color(theme.ColorNameInputBackground))
	track.CornerRadius = height / 2
	track.SetMinSize(fyne.NewSize(0, height))
	if total <= 0 || mins <= 0 {
	...
	seg.SetMinSize(fyne.NewSize(0, height))
	weights := []float32{w}
	segs := []fyne.CanvasObject{seg}
	if rest := 1 - w; rest > 0.001 {
		weights = append(weights, rest)
		segs = append(segs, canvas.NewRectangle(color.Transparent))
	}
	return container.NewStack(track, container.New(newTightRow(weights...), segs...))
}
...
func orgChip(cfg Config, org, amount string, off bool, toggle func(org string)) fyne.CanvasObject {
	var fill color.Color = orgColor(cfg, org)
	...
	chip := container.NewHBox(orgDotIn(fill), container.NewCenter(name))
