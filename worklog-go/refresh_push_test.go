...
// that date is now one item short — and only those. The report's range is the
// payroll period, the 21st through the 20th, so the month a date belongs to on
// the calendar is not the month it belongs to on the report.
func TestAPushDropsOnlyTheRangesHoldingItsDate(t *testing.T) {
	loaded := map[string]bool{
		...
		reportCacheKey("2026-08"): true, // 21 Jul – 20 Aug: holds it
		reportCacheKey("2026-09"): true, // 21 Aug – 20 Sep: does not
	...
	want := map[string][2]string{
		...
		reportCacheKey("2026-08"): {"2026-07-21", "2026-08-20"},
	}
	if len(got) != len(want) {
