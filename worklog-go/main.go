...
func (ui *UI) loadReport(force bool) {
	ui.ensureProjectCache()
	from, to, err := reportBounds(ui.repMonth)
	if err != nil {
...
func (ui *UI) buildLogTab() fyne.CanvasObject {
	...
	recentCard := widget.NewCard("Saved locally, not pushed yet", "",
		container.NewVBox(ui.showPushed, ui.recentBox, widget.NewSeparator(), ui.weekBox))
	...
	return container.NewStack(
		container.NewVScroll(container.NewVBox(logCard, recentCard)), ui.dragLayer)
}
...
func (ui *UI) renderPending() {
	...
	if key := orgLegendWith(ui.cfg, byOrg, ui.orgOff,
		func(n int) string { return fmt.Sprintf("%d commit(s)", n) },
		ui.toggleOrg); key != nil {
	...
	// nothing to log, while the key directly above counts commits waiting under
	// an org that was switched off, is the tab contradicting itself.
	case filteredOut > 0:
		objs = append(objs, widget.NewLabel(fmt.Sprintf(
			"%d unlogged commit(s) hidden by the filter above. Tap an org to bring it back.",
			filteredOut)))
...
func (ui *UI) buildStatusTab() fyne.CanvasObject {
	...
	calCard := widget.NewCard("", "", container.NewBorder(head, ui.calLegend, nil, nil, ui.calBox))
...
func (ui *UI) buildReportTab() fyne.CanvasObject {
	...
	card := widget.NewCard("", "", container.NewVBox(head, ui.repBox))
...
func (ui *UI) drawReport() {
	...
	supportIssues, supportSets, supportBonus := weekendSupportBonus(items)
	...
	stats := container.NewGridWithColumns(3,
		...
		statTile(money(supportBonus), fmt.Sprintf(
			"Weekend support bonus (%d issues → %d set(s))", supportIssues, supportSets)),
	...
	for _, org := range orgsByShare(ui.cfg, byOrg) {
		...
		row := container.New(newRatioRow(0.04, 0.22, 0.54, 0.20),
			orgDot(ui.cfg, org),
			widget.NewLabel(orDefault(org, "unknown org")),
			meterBar(ui.cfg, map[string]int{org: mins}, allMin, 12),
...
func (ui *UI) exportCSV() {
	// Export the project worklog items shown in this payroll period.
	items, _ := ui.itemsFor(reportCacheKey(ui.repMonth))
